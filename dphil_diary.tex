...
\begin{tasklist}
	...
	\task{Write a function to track observed outputs $O_{k-1}$ per sequence}
\end{tasklist}

\paragraph{Model Training and Evaluation}
\begin{tasklist}
	\task{Set up the transformer (6 layers, 6 heads, $d_\text{model}=192$, $d_\text{ffn}=768$)}
	\task{Confirm learned token embeddings + learned absolute positional embeddings, pre-norm residual blocks}
	\task{Implement training loop}
