...
\section{24th February 2016}

\begin{paper}[
	papertitle={DeepRV: Accelerating spatiotemporal inference with pre-trained neural priors~\cite{Navott2025}},
	authors={Navott et al.},
	summary={DeepRV is a decoder-only neural surrogate that replaces GP prior sampling in MCMC.}
]
	\tcblower
	\paragraph{Notes}
	\begin{itemize}
		\item GPs are unfeasible for large datasets due to $\mathcal{O}(N^3)$ scaling. Existing approximations (INLA, sparse/inducing-point GPs, RFFs, VAE-based surrogates like PriorCVAE) trade accuracy for scalability.
		\item Difference to other neural surrogates: omits encoding process, no information bottleneck, leverages factorised stochastic processes directly for training
		\item Key insight: for any stochastic process that decomposes into a latent random vector and a linear transformation, the encoding step can be entirely avoided.
	\end{itemize}
\end{paper}

\section{25th February 2026}

\subsection{Supervisor Meeting Notes}
\begin{meetingsummary}
	\paragraph{Paper Discussion}
	Presentation for last week's questions: \href{https://docs.google.com/presentation/d/1HNfpfqevP0p0xPKaVW9HBpFZLispXNwz7ia5DhiXgAo}{Google Slides}
	Follow-up questions:
	\begin{itemize}
		\item Bayesian Geometry paper: Check MAE calculation? What is $k$? Is the MAE easy/feasible to compute?
	\end{itemize}
	\paragraph{Action Items}
	\begin{itemize}
		\item Contact Dan
		\item Read BSA-TNP paper~\cite{Jenson2025}
		\item Read Tiny Recursive Networks paper~\cite{JolicoeurMartineau2024}
		\item Send Bayesian Transformers papers before next lab meeting + be prepared to present them
		\item Formulate research question for first project (assess DeepRV, tiny recursive mode, gMLP, BSA-TNP)
	\end{itemize}
	\paragraph{First Project}
	Key question: Can we broaden the statement of the Bayesian Geometry paper?
	\begin{itemize}
		\item Verify additional models using Bayesian Wind Tunnels:
		\begin{itemize}
			\item gMLP (see DeepRV paper \cite{Navott2025})
			\item Tiny recursive networks~\cite{JolicoeurMartineau2024}
			\item BSA-TNP~\cite{Jenson2025}
			\item DeepRV~\cite{Navott2025}
		\end{itemize}
	\end{itemize}
	See \href{https://github.com/MLGlobalHealth/dl4bi}{DeepRV GitHub repo}
	\paragraph{Notes}
	\begin{itemize}
		\item \textbf{Think about the overarching question:} why should we do this?
		\item What is the simplest model that can do Bayesian Inference?
		\item Can we do Bayesian Inference (using transformers) for problems where Bayesian Inference is usually hard?
		\item Simulation-based inference
		\item Advanced Topics in Stochastic Machine Learning mini project to be done during term break
		\item[$\rightarrow$] This can become the literature review for the first project/ thesis
	\end{itemize}
\end{meetingsummary}

\section{5th March 2026}

\begin{summary}
	Today's main tasks:
	\begin{itemize}[leftmargin=*]
		\item Start implementing Bijection Wind Tunnel
	\end{itemize}
\end{summary}

\subsection{Task Breakdown: Bijection Wind Tunnel}

\paragraph{Component 1: Create Bijection Dataset}
\begin{tasklist}
	\taskdone{Implement random bijection sampler}
	\taskdone{Write sequence tokenizer in the format $[x_1, y_1, \text{SEP}, x_2, y_2, \text{SEP}, \ldots, x_{19}, \text{SEP}]$}
	\taskdone{Build a DataLoader that generates fresh bijections every batch (no repeats)}
\end{tasklist}

\paragraph{Bayesian Ground Truth}
\begin{tasklist}
	\taskdone{Implement the analytic posterior: uniform over $V-k+1$ unseen outputs at position k}
	\taskdone{Compute $H_\text{Bayes}(k) = log_2(V-k+1)$}
	\taskdone{Write a function to track observed outputs $O_{k-1}$ per sequence}
\end{tasklist}

\paragraph{Model Training and Evaluation}
\begin{tasklist}
	\taskdone{Set up the transformer (6 layers, 6 heads, $d_\text{model}=192$, $d_\text{ffn}=768$)}
	\taskdone{Confirm learned token embeddings + learned absolute positional embeddings, pre-norm residual blocks}
	\taskdone{Implement training loop}
	\task{Evaluate model performance and compare to findings in \cite{Agarwal2026}}
\end{tasklist}

\subsection{Bijection Dataset Implementation}
\begin{itemize}
	\item Implemented as Pytorch datatset to make usage easier and more efficient down the line
	\item Initialisation requires parameters \texttt{V} (vocabulary size, the vocabulary will contain the integers from 1 to V (inclusive)) and \texttt{num\_sequences} (the number of samples to generate). Each sample is a different random bijection.
	\item \textbf{Generating a random bijection:}
	\begin{itemize}
		\item The dataset class stores a list of all the permutations of elements $[1, 2, \ldots, V]$ that have already been used.
		\item[1] To generate a new random bijection, we sample a new random permutation until we get one that is not in the list of used permutations. Token \texttt{1} maps to the first element of the permutation, token \texttt{2} maps to the second element, and so on. We call the sorted list of tokens the \textit{keys} and the corresponding list of mapped values the \textit{values}.
		\item[2] We then stack the key-value pairs together and append a \texttt{SEP} token (value $V+1$) after each pair
		\item[3] Permute the sequence by applying a random permutation to the key-value-SEP tuples (but not the tokens within each tuple)
		\item[4] Flatten the tensor of the first $V-1$ triples to get a sequence of length $3(V-1)$ (the context tokens)
		\item[5] Append the key of the query token and the final \texttt{SEP} token to the end of the sequence. The label for the query token is the value that corresponds to the query key in the bijection. 
	\end{itemize}
\end{itemize}

\section{9th March 2026}

\begin{summary}
	Today's main tasks:
	\begin{itemize}[leftmargin=*]
		\item Continue implementing Bijection Wind Tunnel
		\item Research group meeting (11:00-15:00)
	\end{itemize}
\end{summary}

\subsection{Research Group Meeting}
\begin{meetingsummary}

	\paragraph{DeepRV}
	\begin{itemize}
	\item Bottleneck is data generation
	\begin{itemize}
		\item Can we use an approximation of a Gaussian Process to generate training data? $\rightarrow$ hope is that noise cancels
		\item Simplest version of DeepRV is gMLP
		\item We don’t actually know where the limit of DeepRV is / we don’t know in general what the speed-up of DeepRV is
	\end{itemize}
	\end{itemize}

	\paragraph{Reading (Inducing Points)}
	For inducing points, read Murphy \emph{Probabilistic Machine Learning: Advanced Topics}, Chapter 18 (Gaussian processes).

	\paragraph{Open Problems}
	\begin{itemize}
	\item Train Bottleneck
	\begin{itemize}
		\item $O(n^3)$
		\item Can we train on approximate data?
		\item Goal: we want a cheap way to generate data
	\end{itemize}
	\item Scale
	\begin{itemize}
		\item Can we train a lower dimension DeepRV (gULP)
		\item Inducing DeepRV $\rightarrow$ this is already being worked on
		\item What are the scale limits of DeepRV?
	\end{itemize}
	\item Arbitrary Location
	\begin{itemize}
		\item Fast transformers (Fast / Flash)
		\item ID Embeddings and maximum number of pairs (exploitable to more points)
		\begin{itemize}
		\item Need to give the network the maximum number of points a priori $\rightarrow$ this is a bottleneck
		\end{itemize}
		\item This is about flexibility since
	\end{itemize}
	\end{itemize}

	\paragraph{Other notes}
	\begin{itemize}
	\item \texttt{tinyurl.com/big-data-paradox}
	\item Change points
	\item Spectral mixture kernels $\rightarrow$ very cool and popular but 11 years ago so we can probably do better
	\item Herlands et al. (AISTATS)
	\item Fast Attention
	\item Relevant papers to read:
		\begin{itemize}
			\item BSA-TNP~\cite{Jenson2025}
			\item \href{https://arxiv.org/abs/1503.01057}{Kiss-GP}
		\end{itemize}
	\end{itemize}
\end{meetingsummary}

\section{10th March 2026}

\begin{summary}
	Today's main task:
	\begin{itemize}
		\item Read "LLMs are Bayesian, In Expectation, Not in Realization"~\cite{Chlon2025}
		\item Finish Bijection Wind Tunnel implementation
	\end{itemize}
\end{summary}

\begin{note}
	\begin{itemize}
		\item Finished implementing bijection wind tunnel. Note sure about some of the implementation details though. E.g. the paper~\cite{Agarwal2026} mentions that they only use $10^5$ bijections for training, but later also states that they train for 150k steps with a batch size of 64, which would require $9.6\times 10^6$ bijections if they are all different. I have implemented the dataset to generate fresh bijections every batch (i.e. I use $9.6\times 10^6$ bijections in total), but I am not sure if this is what they did in the paper.
	\end{itemize}
\end{note}

\section{17th March 2026}

\begin{summary}
	Today's main task:
	\begin{itemize}
		\item Implement HMM wind tunnel
	\end{itemize}
\end{summary}

\subsection{HMM Wind Tunnel Implementation}

\begin{note}
	The HMM Wind Tunnel specification is not clear
	\begin{itemize}
		\item Authors use a HMM with $S=5$ hidden states and $V=5$ observation symbols
		\item Each sequence contains a \textbf{10-token header encoding flattened T and E} and \textbf{K observation-prediction pairs, each consisting of the observed symbol $o_t$ and a supervised prediction at the same position for $p(s_t | o_{1:t})$} 
		\item How are $T$ and $E$ encoded into a 10-token header? T is an $S \times S$ matrix and $E$ is $S \times V$, with $S=5$ and $V=5$ this means that both should have 25 elements.
		\item How is the rest of the sequence formatted? Is it $o_i, s_i, o_{i+1}, s_{i+1}, \ldots, o_K, s_K$, where $s_i, s_{i+1}, \ldots, s_K$ are sampled?
	\end{itemize}
\end{note}

\section{18th March 2026}

\begin{summary}
	Today's main tasks:
	\begin{itemize}
		\item Finish HMM wind tunnel implementation
		\item Run full bijection experiment
		\item Start associative recall wind tunnel implementation
	\end{itemize}
\end{summary}
